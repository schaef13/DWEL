;======================================================================
;
function get_point_cloud_script, p_stat
  compile_opt idl2
...
pro evi_point_cloud_batch_doit, event
  compile_opt idl2
  ...
  status=get_point_cloud_script(p_stat)
  ...
  for j_case=0,n_case-1 do begin
    ...
    case_add=strtrim(string(j_case+1),2)
      ...
      ;********************************************
      err=0b
      apply_ptcl_filter_batch, p, pb_stats, pb_meta, pb_info, error=err
      ;********************************************
...
function evi_point_cloud_batch_bannertext
  compile_opt idl2
  ...
  text=[ $
    'Module Description:',$
    '',$
    'Procedure evi_point_cloud_batch is a batch version of evi_point_cloud',$
    'The input data for a number of cases are provided from a script file',$
    'After the script file information is checked you will be asked',$
    'to start running the cases or quit',$
    '',$
    'evi_point_cloud_batch uses filtering and first and second',$
    'derivatives to locate the peaks in EVI shots as a point cloud',$
    'The input file will provide best results if it is AT projected',$
    'Filtering has also been assumed.',$
    'The minimal requirement is that the image has been base fixed',$
    'and also saturation fixed (if needed).',$
    '', $
    'The batch file does NOT prompt for a noise threshold, the EVI Height(m),',$
    'The EVI azimuth to North (assumed zero). Any Save_zero_hits (pure gaps',$
    'recorded) and add_evi (points representing evi base and top added for',$
    'geometry) settings provided are applied to all cases',$
    '',$
    'The first record of the script file is [point_cloud] to check that the right run',$
    'is being made. The information is provided to the program via a script file.',$
    'The script file takes the form of a set of IDL assignment statements "name=value"',$
    'for single variables or "name=[value,value,...]" for arrays. IDL continuation',$
    'lines using $ are used as well as ; to indicate comment lines or comments',$
    'within a record. Full paths are needed for files etc. The outputs are summarised',$
    'in a Log file specified in the script file. Input files are listed in a string',$
    'array form as are ancillary and output files as needed. The script file is first',$
    'checked and if it passes all tests and you agree then the program closes most',$
    'windows and runs in batch mode. If Exit_IDL=1, the batch program closes IDL after',$
    'the run has completed.',$
    '',$
    'Example Script File:',$
    '',$
    '[point_cloud]',$
    ';project_at batch runs for EVI reprocessing - project_at',$
  'n_case=2',$
    "Run_Desc='EVI re-processing 2011 project_at to point clouds'",$
    "log_file='E:\EVI_Processing\initial_test_ptcl.log'",$
    ';',$
  'datafile=[ $',$
    "'E:\EVI_Processing\Lab_Scans\project_at\LabTest_5bdiv_4step_10m_ND200_ht167_nhem110_cube_nu_basefix_nu_satfix_pfilter_at_project.img',$",$
    "'E:\EVI_Processing\Lab_Scans\project_at\LabTest_5bdiv_4step_80m_ND300_ht158_nhem110_cube_nu_basefix_nu_satfix_pfilter_at_project.img' $",$
    ']',$
    ';',$
  'anc_name=[ $',$
    "'E:\EVI_Processing\Lab_Scans\project_at\LabTest_5bdiv_4step_10m_ND200_ht167_nhem110_cube_nu_basefix_nu_satfix_pfilter_at_project_extrainfo.img',$",$
    "'E:\EVI_Processing\Lab_Scans\project_at\LabTest_5bdiv_4step_80m_ND300_ht158_nhem110_cube_nu_basefix_nu_satfix_pfilter_at_project_extrainfo.img' $",$
    ']',$
    ';',$
  'outfile=[ $',$
    "'E:\EVI_Processing\Lab_Scans\pt_cld\LabTest_5bdiv_4step_10m_ND200_ht167_nhem110_cube_nu_basefix_nu_satfix_pfilter_at_project_ptcl.log',$",$
    "'E:\EVI_Processing\Lab_Scans\pt_cld\LabTest_5bdiv_4step_80m_ND300_ht158_nhem110_cube_nu_basefix_nu_satfix_pfilter_at_project_ptcl.log' $",$
    ']',$
    ';',$
  'Exit_IDL=0 ;if exit_idl=1 then the batch closes idl on completion',$
  '',$
    'The output files are ascii files. One is a header file with run',$
    'information and the other the point cloud. The file extent for',$
    'the header is .pch (point cloud header) and for the point cloud',$
    'the extent is .log. The point cloud data is in CSV format.',$
    '',$
    'The header (metadata file) contins the following information:',$
    '',$
    '[EVI_Point_Cloud_Metadata]',$
    'Processing_Date_Time',$
    'Description',$
    'Input_Path',$
    'Input_File',$
    ...
    'Estimated_Ratio',$
    '',$
    'The point cloud data are stored as Nrecs+3 CSV data records.',$
    'There are three headers and Nrecs return data sets.',$
    'The first few lines of a typical file could be as follows:',$
    '',$
    '[EVI Point Cloud Data]',$
    'Run made at: Sun Apr 13 00:26:12 2008',$
    'X,Y,Z,Intensity,Return_Number,Number_of_Returns,I2,range,theta,phi,Sample,Line,Band',$
    '0.000,29.620,4.167,88.6667,1,1,56.3723,29.72,85.20,0.00,299,1,490',$
    '0.000,29.665,4.015,236.6667,1,1,149.9540,29.76,85.50,0.00,300,1,490',$
    '(etc to Nrecs of EVI returns)',$
    '',$
    'X,Y,Z are in metres with the point on the ground under the EVI',$
    'as 0,0,0. If the EVI azimuth to North has been set the orientation',$
    'will be with the Y axis as North. If GPS provides UTM coordinates',$
    'and height above datum these will be the offsets for the EVI data',$
    'The intensity is in the units of the image and is the value at the',$
    'peak of the return. The records contain the return number and the',$
    'total number of returns for the shot. Sample, Line and Band allow',$
    'you to find the point in the original image.',$
    '',$
    'NOTE: from the batch runs the information will normally be power data,',$
    'base of evi as (0,0,0) and y axis to evi azimuth zero. The EVI base and',$
    'height "dummy" hits will generally not be added. Future changes will be',$
    'made by a separate program as there are still some issues but the present',$
    'the current output provides the information needed for the changes.',$
    '' $
    ]
...
pro evi_point_cloud_batch_action_go, event
  compile_opt idl2
  ...
  evi_point_cloud_batch_doit, event
  
  return
  
end

pro evi_point_cloud_batch_action_exit, event
  compile_opt idl2
...
pro evi_point_cloud_batch_resize, event
  compile_opt idl2
  ...
  if (error_status ne 0) then begin
    ...
    info_text=[strtrim(string('Catch trapped an Error !'),2),$
      ...
      format='(a,i5)'),2),$
      strtrim('Last Message: '+strtrim(out,2),2)]
    result=dialog_message(info_text,/error,title='Error in evi_point_cloud_batch_resize', $
      dialog_parent=event.top)
    goto, cleanup
  endif
...
pro evi_point_cloud_batch_cleanup,top
  compile_opt idl2
...
pro dwel_point_cloud_batch, event
  compile_opt idl2
  
  envi, /restore_base_save_files
  
  envi_set_path,path_to,/no_set
  error_log_file=strtrim(path_to,2)+'\point_cloud_batch_errors.log'
  
  envi_batch_init, log_file=error_log_file
  ...
  wb_base=widget_base(title='point_cloud EVI Data Cube',/column,$
  ...
  intro_text=evi_point_cloud_batch_bannertext()
  ...
  w_go=widget_button(wb_action,value='Go',uvalue='evi_point_cloud_batch_action_go',$
    event_pro='evi_point_cloud_batch_action_go',/dynamic_resize)
  w_exit=widget_button(wb_action,Value='Exit',uvalue='evi_point_cloud_batch_action_exit',$
    event_pro='evi_point_cloud_batch_action_exit',/dynamic_resize)
  ...
  ;call xmanager
  xmanager,'evi_point_cloud_batch',wb_base, $
    event_handler='evi_point_cloud_batch_resize',$
    cleanup='evi_point_cloud_batch_cleanup',/no_block
