...
function AlignedDataCube, OldDataCube_File, OldAnc_File, $
  AlignedMaskFile, DataCube_File, Wavelength, nadirelevshift
  ...
  envi_open_file, OldDataCube_File, /no_realize, r_fid=oldfid
  if oldfid eq -1 then begin
    print,strtrim('Cannot open input file',2)
    print,'Input File: '+strtrim(OldDataCube_File,2)
    return, -1
  endif
  envi_open_file, OldAnc_File, /no_realize, r_fid=oldancfid
  if oldancfid eq -1 then begin
    print,strtrim('Cannot open input file',2)
    print,'Input File: '+strtrim(OldAnc_File,2)
    return, -1
  endif
  
  envi_file_query, oldancfid, data_type=anc_dtype, dims=anc_dims, $
    interleave=anc_ftype, nb=anc_nb, ns=anc_ns, nl=anc_nl
  envi_file_query, oldfid, data_type=dtype, dims=dims, $
    interleave=ftype, nb=nb, ns=ns, nl=nl
  ; read the encoder data from the old ancillary file
  OldScanEncoder = envi_get_data(dims=anc_dims, fid=oldancfid, pos=2)
  OldRotaryEncoder = envi_get_data(dims=anc_dims, fid=oldancfid, pos=3)
  oldmask = envi_get_data(dims=anc_dims, fid=oldancfid, pos=6)
  ; correct the shift of nadir
  OldScanEncoder = ((OldScanEncoder - nadirelevshift) + 524288) mod 524288
...
;  memspace = h5s_create_simple([DWEL_MetaInfo.NoSamplesPerShot, 1])
  ...
  DataArray = intarr(aligned_ns, nb)
...
;  BlankVec = bytarr(DWEL_MetaInfo.NoShotsPerScan)
  ZeroWaveform = intarr(nb)
  for i = 0, aligned_nl-1, 1 do begin    
    for j = 0L, aligned_ns-1, 1  do begin
      if (AlignedMask[j, i] eq 0) or (shotind ge ns*nl) then begin
      mask = 0
...
;      Waveform = h5d_read(waveset, file_space=wave_space, MEMORY_SPACE = memspace)
      il = fix(shotind/ns)
      is = shotind mod ns
      while (oldmask[is, il] eq 0) do begin
        shotind = shotind + 1
        il = fix(shotind/ns)
        is = shotind mod ns
      end
      Waveform = envi_get_slice(/bip, fid=oldfid, line=il, pos=indgen(nb), xe=is, xs=is)
...
;      ShotZen = double(262144 - ScanEncoder) / double(524288) * 2 * 180.0
      ...
      ShotZen = double(262144 - ScanEncoder) / double(524288) * 2 * 180.0
      RotaryEncoder = OldRotaryEncoder[is,il]
...
;  H5_CLOSE
  ...
  return, {samples:data_dims[1], lines:aligned_nl, $
    ...
    TotalNoScans:nl, NoShotsPerScan:ns, NoSamplesPerShot:nb, $
    NoScanPerRotation:fix(nl * 524288.0 / abs(max(OldRotaryEncoder)-min(OldRotaryEncoder)))}
  
end

pro DWEL_AlignCube_cmd, DWEL_H5File, OldDataCube_File, OldAnc_File, AlignedMaskFile, DataCube_File, Wavelength, DWEL_Height, $
...
;  DWEL_MetaInfo = AlignedCheckDWEL(DWEL_H5File, Wavelength, nadirelevshift)
  HeaderInfo = AlignedDataCube(OldDataCube_File, OldAnc_File, AlignedMaskFile, DataCube_File, Wavelength, nadirelevshift)
...
;    byteorder:0, wavelengthunit:'metres', range:120.0}
  ...
  'Scans per Complete Rotation = '+strtrim(string(HeaderInfo.NoScanPerRotation),2),$
  'Number of Scans selected = '+strtrim(string(HeaderInfo.TotalNoScans),2),$
  'Number of Shots per Scan = '+strtrim(string(HeaderInfo.NoShotsPerScan),2),$
  'Number of Samples per Shot = '+strtrim(string(HeaderInfo.NoSamplesPerShot),2),$
  ...
  'Actual scans completed = '+strtrim(string(HeaderInfo.TotalNoScans),2)]
