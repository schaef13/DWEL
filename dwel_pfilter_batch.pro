;======================================================================
;
function get_pfilter_script, p_stat
compile_opt idl2
...
pro evi_pfilter_batch_doit, event
compile_opt idl2
...
status=get_pfilter_script(p_stat)
...
function evi_pfilter_batch_bannertext
compile_opt idl2
...
'Module Description:',$
' ',$
'Procedure evi_pfilter_batch is a batch version of evi_pulse_filter',$
...
' ',$
'The evi_pfilter_batch cases each computes a raster ENVI image from',$
'EVI sat_fixed data, which may or may not be projected.',$
'There are two additional optional output images.',$
'These are the B (intensity) and R (correlation) values used in the filter.',$
' ',$
'The first record of the script file is [pfilter] to keep then possibility of',$
'different types of operation being in one file later.',$
...
' ',$
'Default values for filter parameters will usually be used, however there',$
'are optional flags which may be specified in the script file to set values.',$
'These are:',$
'	min_index [the start index of the section of mean pulse array to be used,DEF=16]',$
'	max_index [the end index of the section of mean pulse array to be used, DEF=236]',$
'	b_thresh [the intensity threshold,DEF=9.5]',$
'	r_thresh [the correlation threshold,DEF=0.05]',$
' ',$
'Example Script File:',$
' ',$
'[pfilter]',$
';Test case for the data pfilter batch runs',$
'n_case=3',$
"Run_Desc='EVI Tumbarumba Site PSP06 Pulse Filter Runs to test batch program'",$
"log_file='d:\pfilter_Test\EVITumba_Test_Case_PSP06.log'",$
';',$
';This is data from day 3 of a field trip',$
';',$
'datafile=[ $',$
"'d:\pfilter_Test\ND0\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_satfix.img',$",$
"'d:\pfilter_Test\ND0\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_satfix.img',$",$
"'d:\pfilter_Test\ND0\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_satfix.img' $",$
']',$
'ancfile=[ $',$
"'d:\pfilter_Test\ND0\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_satfix_ancillary.img',$",$
"'d:\pfilter_Test\ND0\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_satfix_ancillary.img',$",$
"'d:\pfilter_Test\ND0\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_satfix_ancillary.img' $",$
']',$
'outfile=[ $',$
"'d:\pfilter_Test\ND0\pfilter\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_satfix_pfilter.img',$",$
"'d:\pfilter_Test\ND0\pfilter\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_satfix_pfilter.img',$",$
"'d:\pfilter_Test\ND0\pfilter\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_satfix_pfilter.img' $",$
']',$
'Exit_IDL=1',$
...
pro evi_pfilter_batch_action_go, event
compile_opt idl2
...
evi_pfilter_batch_doit, event

return

end

pro evi_pfilter_batch_action_exit, event
compile_opt idl2
...
pro evi_pfilter_batch_resize, event
compile_opt idl2
...
if (error_status ne 0) then begin
   ...
   info_text=[strtrim(string('Catch trapped an Error !'),2),$
              ...
              format='(a,i5)'),2),$
              strtrim('Last Message: '+strtrim(out,2),2)]
   result=dialog_message(info_text,/error,title='Error in evi_pfilter_batch_resize', $
   dialog_parent=event.top)
   goto, cleanup
endif
...
pro evi_pfilter_batch_cleanup,top
compile_opt idl2
...
pro dwel_pfilter_batch, event
compile_opt idl2

envi, /restore_base_save_files

envi_set_path,path_to,/no_set
error_log_file=strtrim(path_to,2)+'\pfilter_batch_errors.log'
...
wb_base=widget_base(title='pfilter EVI Data Cube',/column,$
...
intro_text=evi_pfilter_batch_bannertext()
...
w_go=widget_button(wb_action,value='Go',uvalue='evi_pfilter_batch_action_go',$
event_pro='evi_pfilter_batch_action_go',/dynamic_resize)
w_exit=widget_button(wb_action,Value='Exit',uvalue='evi_pfilter_batch_action_exit',$
event_pro='evi_pfilter_batch_action_exit',/dynamic_resize)
...
;call xmanager
xmanager,'evi_pfilter_batch',wb_base, $
event_handler='evi_pfilter_batch_resize',$
cleanup='evi_pfilter_batch_cleanup',/no_block
