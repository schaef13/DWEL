...
function get_project_at_script, p_stat
  compile_opt idl2
...
;======================================================================
pro evi_proj_at_batch_doit, event
  compile_opt idl2
  ...
  status=get_project_at_script(p_stat)
...
function evi_proj_at_batch_bannertext
  compile_opt idl2
  ...
  text=[ $
    'Module Description:',$
    ' ',$
    'Procedure evi_proj_at_batch is a batch version of evi_project_at',$
    ...
    ' ',$
    'The evi_proj_at_batch cases each compute a raster ENVI image from',$
    'EVI Cube data using the encoder positions and a specified projection.',$
    'There are two out put images. One is the projected image',$
    'The other is a file of infomation data planes.',$
    ' ',$
    'The image is gridded using the AT projection and defines',$
    'bins in terms of projected zenith and azimuth resolution.',$
    'ONLY the Andrieu Transpose (AT) projection is produced by project_at',$
    ' ',$
    'The output ancillary planes include the number of shots in', $
    'each output cell, the zenith and azimuth angles for each cell centre,',$
    'the data Mask (valid data), the Waveform mean, the range weighted',$
    'mean, the range^2 weighted mean, the Star and the RMS star.',$
    ' ',$
    'The procedure reduces the EVI data by removing redundent shots',$
    'and provides a useful base for later processing.',$
    ' ',$
    'The first record of the script file is [project_at] to keep then possibility of',$
    'different types of operation being in one file for later development.',$
    'The script file then takes the form of a set of IDL assignment statements "name=value"',$
    'IDL continuation lines are allowed as well as ; to indicate comment lines or comments',$
  'within a record. Full paths are needed for files etc',$
    ' ',$
    'Example Script File:',$
    ' ',$
    '[project_at]',$
    ';Example for an AT project batch run',$
  'n_case=3',$
    "Run_Desc='EVI Tumbarumba Site PSP06 Projection Runs to document batch program'",$
    "log_file='d:\Proj_Test\EVITumba_Test_Case_PSP06_Proj.log'",$
    ';',$
  'image=[ $',$
    "'d:\Proj_Test\ND0\pfilter\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_nu_satfix_pfilter.img',$",$
    "'d:\Proj_Test\ND0\pfilter\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_nu_satfix_pfilter.img',$",$
    "'d:\Proj_Test\ND0\pfilter\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_nu_satfix_pfilter.img' $",$
    ']',$
    'anc_name=[ $',$
    "'d:\Proj_Test\ND0\satfix\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_nu_satfix_ancillary.img',$",$
    "'d:\Proj_Test\ND0\satfix\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_nu_satfix_ancillary.img',$",$
    "'d:\Proj_Test\ND0\satfix\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_nu_satfix_ancillary.img' $",$
    ']',$
    'outfile=[ $',$
    "'d:\Proj_Test\ND0\at_project\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_nu_satfix_pfilter_at_project.img',$",$
    "'d:\Proj_Test\ND0\at_project\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_nu_satfix_pfilter_at_project.img',$",$
    "'d:\Proj_Test\ND0\at_project\EVITumba_Scan10_PSP06_5mrad_ND00_cube_basefix_nu_satfix_pfilter_at_project.img' $",$
    ']',$
    'output_resolution=[5.0,5.0,5.0] ;output resolution is mrad',$
  'Max_Zenith_Angle=[125.0,125.0,125.0] ;max zenith angle is in degrees',$
  'Exit_IDL=1     ;if exit_idl=1 the program closes idl when complete',$
  'delete_input=0  ;if delete_input=1 the input files are deleted to save space',$
  ' ' $
    ]
...
pro evi_proj_at_batch_action_go, event
  compile_opt idl2
  ...
  evi_proj_at_batch_doit, event
  
  return
  
end

pro evi_proj_at_batch_action_exit, event
  compile_opt idl2
...
pro evi_proj_at_batch_resize, event
  compile_opt idl2
  ...
  if (error_status ne 0) then begin
    ...
    info_text=[strtrim(string('Catch trapped an Error !'),2),$
      ...
      format='(a,i5)'),2),$
      strtrim('Last Message: '+strtrim(out,2),2)]
    result=dialog_message(info_text,/error,title='Error in evi_proj_at_batch_resize')
    goto, cleanup
  endif
...
pro evi_proj_at_batch_cleanup,top
  compile_opt idl2
...
pro dwel_proj_at_batch, event
  compile_opt idl2
  
  envi, /restore_base_save_files
  
  envi_set_path,path_to,/no_set
  error_log_file=strtrim(path_to,2)+'\batch_projection_errors.log'
  ...
  wb_base=widget_base(title='Projecting EVI Cube spatial coords',/column,$
  ...
  intro_text=evi_proj_at_batch_bannertext()
  ...
  w_go=widget_button(wb_action,value='Go',uvalue='evi_proj_at_batch_action_go',$
    event_pro='evi_proj_at_batch_action_go',/dynamic_resize)
  w_exit=widget_button(wb_action,Value='Exit',uvalue='evi_proj_at_batch_action_exit',$
    event_pro='evi_proj_at_batch_action_exit',/dynamic_resize)
  ...
  ;call xmanager
  xmanager,'evi_proj_at_batch',wb_base, $
    event_handler='evi_proj_at_batch_resize',$
    cleanup='evi_proj_at_batch_cleanup',/no_block
